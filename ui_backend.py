...
def generate_images(
    ...
    command = "pvpython "+ os.path.join(current_dir, "cli.py") +" --input \"" + str(input) + "\" --output \""+  str(output)+ "\" --size " +  str(size)+ " --numberRotations " +  str(numberRotations) + " --numberImages " +  str(numberImages) + " --anglePerRotation " +  str(anglePerRotation) + " --distanceOrRadio " +  str(distanceOrRadio)+ " --initMovement "+  str(initMovement) + " --endMovement "+  str(endMovement)+ " --cantStepMovement " +  str(cantStepMovement)
